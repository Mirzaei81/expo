...
export default function TabTwoScreen() {
  ...
  const [statusMessage, setStatusMessage] = useState("Your picture is being uploaded now and then GPT will take a minute or two. Please wait!")
...
function _makeButton(item){
  const websearch = 'https://result.websearch-via-camera.com/en/'.concat(item.item.value);
  return (
    <TouchableOpacity style={{marginTop:20, marginBottom:20}} href={websearch}>
      <Button  title={item.item.value}>
      </Button>
      </TouchableOpacity>
  );
}
...
//   }
  ...
  function upload() {
    ...
    fetch(url, {
      ...
      .catch((error) => {
        setStatusMessage("Error uploading this file. Try with a different image!");
        setSpinnerVisible(false);
...
const presetImage = (height) => {
  if (height==750){
    setQueries(['Avocado toast recipe', 'How to make avocado spread for bread', 'Healthy breakfast ideas', 'Egg and avocado toast', 'Avocado toast with boiled eggs', 'Simple avocado breakfast', 'Vegetarian breakfast recipes', 'Avocado and tomato on toast', 'Avocado toast variations', 'Nutritious breakfast with avocado and eggs']);
    setQueriesTwo(['Slices of bread with avocado spread', 'A half of an avocado with the pit still inside', 'Two halves of a boiled egg with pepper sprinkled on top', 'Fresh cherry tomatoes', 'Wooden cutting board', 'Sprinkle of pepper and spices around the board', 'Sprigs of dill']);
  }
  if (height==281){
    setQueries(['Sea otter with pup','Mother sea otter with baby', 'Sea otter habitat and lifestyle', 'Cute sea otter family', 'Sea otter conservation status']);
    setQueriesTwo(['An adult sea otter', 'A baby sea otter (pup)', 'Water with ripples']);
  }
  if (height==306){
    setQueries(['Ten Commandments in the Bible', 'Bible verses about not killing', 'Thou shalt not steal verse', 'Exodus 20 commandments text', 'Biblical text Thou shalt not bear false witness', 'Scripture on not committing adultery', 'Holy Bible commandments passage', 'Christian ethical teachings in Bible', 'Old Testament rules and commandments', 'Decalogue text in Christianity']);
    setQueriesTwo(['A printed page from bible.', 
...
return (
    ...
    <ScrollView horizontal={false} contentContainerStyle={{ flexGrow: 1 }} style={styles.container}>
      ...
      {!queries && !image && <View style={{marginLeft:10, marginRight:10, alignItems:'center'}} className='mt-7'>
        ...
        <Text className="text-white">Pick An Image From Gallery</Text>
          ...
          <TouchableOpacity style={{marginBottom:20}} onPress={() => presetImage(281)}>
            <Image style={{margin: "auto", width:500,height:281,maxWidth:"100%", maxHeight:281}} source={{
              uri: 'https://websearch-via-camera.com/camera/bcubjmsofu123pvzdyjn.jpeg',
            }}/>
          </TouchableOpacity>
          <TouchableOpacity style={{marginBottom:20}} onPress={() =>presetImage(306)}>
            <Image style={{margin: "auto", width:500,height:306,maxWidth:"100%", maxHeight:306}} source={{
              uri: 'https://websearch-via-camera.com/camera/ebj8nlazyhgz3ucs6e6n.jpeg',
            }}/>
          </TouchableOpacity>
          <TouchableOpacity style={{marginBottom:20}} onPress={() =>presetImage(750)}>
            <Image style={{margin: "auto", width:500,height:750,maxWidth:"100%", maxHeight:750}} source={{
              uri: 'https://websearch-via-camera.com/camera/mlkua19iskzugi1u27ch.jpeg',
            }}/>
          </TouchableOpacity>
      ...
      {!queries && image && <View><Spinner visible={spinnerVisible}/>
      <Text style={{padding:20}}>{statusMessage} </Text>
      <TouchableOpacity href="https://websearch-via-camera.com/camera/"><Button title="Start over"></Button></TouchableOpacity>
       <PureCanvas ref={canvasRef} /> 
       </View>
       }
      {/* {!queries && image && <Button title="Reset" onPress={() => removeImage()} />} */}
      {queries && queries.length > 0 && <Text style={{margin: "20px", fontSize: "large",fontWeight: "bold"}}>Search Intents:</Text>}
      {queries && queries.length > 0 && <FlashList estimatedItemSize={100} data={[{value:'Title Text'}, {value:'Title Text2'}]} renderItem={_makeButton} />}

      {queriesTwo && queriesTwo.length > 0 && <Text style={{margin: "20px", fontSize: "large",fontWeight: "bold"}}>Objects in the photo:</Text>}
        {queriesTwo && queriesTwo.length > 0 && queriesTwo.map(makeButton, this)}
      {(queriesTwo || queries) && <Text style={{margin: "20px", fontSize: "large",fontWeight: "bold"}}>Want to restart?</Text>}
      {(queriesTwo || queries) && <TouchableOpacity href="https://websearch-via-camera.com/camera/"><Button title="Start over"></Button></TouchableOpacity>
}
